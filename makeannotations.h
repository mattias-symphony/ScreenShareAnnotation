...
static LRESULT CALLBACK makeAnnotationsWndProc( HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
    ...
    switch( message ) {
        ...
        case WM_PAINT: {
            ...
            HBRUSH t =CreateSolidBrush( transparent );

            RECT r;
            GetClientRect( hwnd, &r  );
            FillRect( backbuffer, &r, t );
            ...
            HDC dc2 = BeginPaint( data->hwnd2, &ps );
            BitBlt( dc2, 0, 0, data->bounds.right - data->bounds.left, data->bounds.bottom - data->bounds.top, 
                    backbuffer, 0, 0, SRCCOPY );
            EndPaint( data->hwnd2, &ps );
            UpdateWindow( data->hwnd2 );
            InvalidateRect( data->hwnd2, NULL, TRUE );
        } break;
...
static LRESULT CALLBACK makeAnnotationsWndProc2( HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
    struct MakeAnnotationsData* data = (struct MakeAnnotationsData*) GetWindowLongPtrA( hwnd, GWLP_USERDATA );
    switch( message ) {
        case WM_NCCREATE: {
            if( EnableNonClientDpiScalingPtr ) {
...
int makeAnnotations( HMONITOR monitor, int lang ) {
    COLORREF transparent = RGB( 0, 0, 0 );
    ...
    WNDCLASSW wc = { 
        ...
        icon,                                   // hIcon
        NULL,                                   // hCursor
        (HBRUSH) CreateSolidBrush( transparent ), // hbrBackground
        NULL,                                   // lpszMenuName
    ...
    WNDCLASSW wc2 = { 
        CS_OWNDC | CS_HREDRAW | CS_VREDRAW,     // style
        (WNDPROC) makeAnnotationsWndProc2,       // lpfnWndProc
        0,                                      // cbClsExtra
        0,                                      // cbWndExtra
        GetModuleHandleA( NULL ),               // hInstance
        icon,                                   // hIcon
    ...
    // Create window
    HWND hwnd = CreateWindowExW( WS_EX_TOOLWINDOW | WS_EX_LAYERED | WS_EX_TOPMOST, wc.lpszClassName, 
        localization[ lang ].title, WS_POPUPWINDOW | WS_VISIBLE, x, y, width, height,
        NULL, NULL, GetModuleHandleW( NULL ), 0 );

    HWND hwnd2 = CreateWindowExW( WS_EX_TOOLWINDOW |WS_EX_LAYERED | WS_EX_TOPMOST, wc2.lpszClassName, 
        localization[ lang ].title, WS_POPUPWINDOW | WS_VISIBLE, x, y, width, height,
        NULL, NULL, GetModuleHandleW( NULL ), 0 );
    ...
    makeAnnotationsData.penIndex = 2;
    ...
    makeAnnotationsData.menuMarginH = 1;
    makeAnnotationsData.menuMarginV = 1;
...
*/
    ...
    makeAnnotationsData.hwnd2 = hwnd2;

    // Set window transparency

    SetWindowLongPtrA( hwnd2, GWLP_USERDATA, (LONG_PTR)&makeAnnotationsData );
    SetLayeredWindowAttributes( hwnd, transparent, 1, LWA_ALPHA /* | LWA_COLORKEY */ );
    
    UpdateWindow( hwnd );

    SetLayeredWindowAttributes( hwnd2, transparent, 160, LWA_ALPHA  | LWA_COLORKEY );
    UpdateWindow( hwnd2 );
    ...
    while( running )  {
        // Process messages for each window
            ...
            while( PeekMessageA( &msg, hwnd2, 0, 0, PM_REMOVE ) ) {
                TranslateMessage( &msg );
                DispatchMessage( &msg );
            }
